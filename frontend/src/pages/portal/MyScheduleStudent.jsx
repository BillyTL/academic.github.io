...
import { studentSchedule } from '../../services/scheduleService';

const DAY_ORDER = ['Lunes','Martes','Miercoles','Jueves','Viernes','Sabado'];

export default function MyScheduleStudent() {
  const { toggleSidebar } = useOutletContext();
  const { data, loading } = useFetch(studentSchedule, []);
                ...
                {grouped[day].map(s => (
                  <div key={s.id} className="flex items-center gap-4 p-3 bg-bglight rounded-card">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-[13px] text-textmuted">{s.course_name} · Prof. {s.teacher_name}</div>
                    </div>
