...
const state = {
  ...
  inscriptions: [],
  users: [],
  attendance: [],   // FIX: was {}, now array to match API shape
  schedules: [],
  nextId: 100
};

// FIX: robust apiFetch — reads body as text first so PHP errors don't crash JSON.parse
...
// FIX: use Promise.allSettled so one failed resource doesn't wipe all others.
// FIX: accept empty arrays from the API (removed the && x.length condition).
...
// FIX: use id as option value so selects work correctly even if two courses share a similar name
...
// FIX: deleteStudent now calls the API
...
// FIX: saveStudent now calls the API (update only — creation goes through Inscripciones)
...
// FIX: deleteTeacher now calls the API
...
// FIX: saveTeacher now calls the API
...
// FIX: deleteSubject now calls the API
...
// FIX: saveSubject now calls the API
async function saveSubject(){
  const name=document.getElementById('sub-name').value.trim();
  if(!name){document.getElementById('sub-name').parentElement.classList.add('has-error');return;}
  document.getElementById('sub-name').parentElement.classList.remove('has-error');
...
// FIX: deleteCourse now calls the API
...
// FIX: saveCourse now calls the API
...
// FIX: deleteSchedule now calls the API
...
// FIX: saveSchedule now calls the API
...
// FIX: deleteGrade now calls the API
...
// FIX: saveGrade now calls the API
...
// ======= ATTENDANCE =======
// FIX: attendance is now an array; attendance page shows records from API
function renderAttendance(){
  ...
  // Also populate from students list so every student shows up for the chosen date/course
...
// FIX: deletePayment now calls the API
...
// FIX: savePayment now calls the API
...
// FIX: deleteUser now calls the API
...
// FIX: saveUser now calls the API
