...
// ============================================================
//  ui.js — Funciones puramente de frontend (render, modales,
//          DOM). No llaman a apiFetch en ningún momento.
// ============================================================
...
// openStudentModal sólo arma el modal con datos ya en `state`,
// sin disparar ninguna petición HTTP.
function openStudentModal(id) {
  document.getElementById('s-id').value = id || '';
  if (id) {
    const s = state.students.find(x => String(x.id) === String(id));
...
function openTeacherModal(id) {
  ...
  ['t-user', 't-specialty', 't-email', 't-pass', 't-shift'].forEach(id =>
    document.getElementById(id).parentElement.classList.remove('has-error')
  );
...
function openSubjectModal(id) {
  ...
  document.getElementById('sub-teacher').innerHTML =
    '<option value="">Seleccione un docente</option>' + teacherOptions('');
  if (id) {
...
const roleColors = {
  ...
  Director:      'badge-blue',
  Secretaria:    'badge-success',
  Docente:       'badge-blue',
  Tesorero:      'badge-warning'
};
...
function applyReportFilter() {
  ...
  if (currentReport === 'notas') {
    ...
    tbody = state.payments.map(p =>
      `<tr><td>${p.student}</td><td>${p.concept}</td><td><strong>Bs. ${p.amount}</strong></td><td>${p.method || '—'}</td><td>${p.date || '—'}</td><td>${badgeStatus(p.status)}</td></tr>`
    ).join('');
  }
...
function renderStudents(q) {
  // Implementación idéntica a la original (renderizado puro desde state).
  if (q === undefined) q = (document.querySelector('#page-students .search-box input') || {}).value || '';
  const filtered = state.students.filter(s => !q || s.name.toLowerCase().includes(q.toLowerCase()));
  const tbody = document.getElementById('students-tbody');
  if (!filtered.length) { tbody.innerHTML = emptyRow(8); return; }
  tbody.innerHTML = filtered.map(s => `<tr>
    <td style="font-size:11px;color:var(--gray-400)">${s.id}</td>
    <td><div style="font-weight:500">${s.name}</div></td>
    <td>${s.course || '—'}</td><td>${s.shift || '—'}</td>
    <td>${badgeStatus(s.status || 'Activo')}</td>
    <td>${actionBtns(`editStudent('${s.id}')`, `deleteStudent('${s.id}')`)}</td>
  </tr>`).join('');
}